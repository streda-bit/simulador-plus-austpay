...
// 1. Atualize a constante de elementos (els)
const els = {
    amount: document.getElementById('amount'),
    brand: document.getElementById('brand'),
    method: document.getElementById('method'),
    ...
    receiveAmount: document.getElementById('receiveAmount'),
    chargeAmount: document.getElementById('chargeAmount'),
    shareBtn: document.getElementById('shareBtn') // Adicionado
};

// 2. Adicione o evento de clique dentro da sua função init()
function init() {
    // ... seu código de criação das opções 1x até 21x ...

    // Adicione esta linha:
    els.shareBtn.addEventListener('click', shareSimulation);

    // ... restante dos seus eventos (input, change, etc) ...
}

// 3. Crie a nova função de compartilhar no final do arquivo
function shareSimulation() {
    const brandName = els.brand.options[els.brand.selectedIndex].text;
    const methodType = els.method.value === 'credit' ? `${els.installments.value}x` : els.method.options[els.method.selectedIndex].text;
    
    const texto = `Austpay Simulação:
${brandName} - ${methodType}
Cobrar do cliente: ${els.chargeAmount.innerText}
Você recebe: ${els.receiveAmount.innerText}`;

    navigator.clipboard.writeText(texto).then(() => {
        // Feedback visual no botão
        const originalText = els.shareBtn.innerText;
        els.shareBtn.innerText = "✅ Copiado!";
